...
{
	return check_type(parser, TOKEN_IDENTIFIER) || check_type(parser, TOKEN_STRING_LITERAL) || check_type(parser, TOKEN_INTEGER_LITERAL) || check_type(parser, TOKEN_DECIMAL_LITERAL) || check_keyword(parser, KW_NIL);
}

bool is_literal(Parser *parser)
{
	return check_type(parser, TOKEN_STRING_LITERAL) || check_type(parser, TOKEN_INTEGER_LITERAL) || check_type(parser, TOKEN_DECIMAL_LITERAL) || check_keyword(parser, KW_NIL);
}
...
// Check if current token is of the expected type and advance if it is, otherwise print error
// message and exit
Token consume(Parser *parser, TokenType token_type, char *error_msg)
{
	if (!check_type(parser, token_type))
	{
		...
		// 				current_token(parser)->keyword, current_token(parser)->val, peek(parser)->type,
		// 				peek(parser)->val, error_msg);
		// #else
...
void func_params_n(Parser *parser, SymtableItem *item, Symtable *local_table, void_stack_t *stack)
{
	...
	symtable_add_symbol(local_table, in_param_id, SYMTABLE_VARIABLE, true, false, true);
...
void check_call_param(Parser *parser, SymtableParam *param, Token *first, Token *second)
{
	if (!is_term(parser))
	{
	...
	{
		exit_with_error(SEMANTIC_ERR_CALL, "Parameter %s doesn't have out definition", first->val);
	}
	...
	// {
	// 	exit_with_error(SEMANTIC_ERR_CALL, "Parameter %s doesn't match the out definition", first);
	// }
	...
	if (first->type == TOKEN_IDENTIFIER)
	{
		...
		else
		{
			...
			if (!compare_symtable_item_types(param_variable->data->variable.identifier_type,
	...
	else if (is_literal(parser))
	{
		...
		{
			exit_with_error(SEMANTIC_ERR_CALL, "Parameter %s type doesn't match the definition",
							first->val);
		}
	}
	else
...
void call_params(Parser *parser, SymtableItem *item, SymtableParam *param)
{
	...
	check_call_param(parser, param, &first, second);
	// printf("after, %d | '%s'\n", current_token(parser)->type, current_token(parser)->val);
...
bool if_statement(Parser *parser)
{
	...
	if (match_keyword(parser, KW_LET, false))
	{
		...
		SymtableItem *result = symtable_get(parser->global_table, var_id);
		...
		if (result == NULL)
		{
			...
			while (i >= 0)
			{
				result = symtable_get(parser->local_tables_stack->items[i], var_id);
				if (result != NULL)
				{
					var_exists = true;
					break;
...
bool statement(Parser *parser)
{
	...
	else if (match_keyword(parser, KW_LET, true) || match_keyword(parser, KW_VAR, true))
	{
		...
		if (match(parser, TOKEN_ASSIGN, false))
		{
			...
			if (check_type(parser, TOKEN_IDENTIFIER) && peek(parser)->type == TOKEN_LPAREN)
			{
				...
				if (!compare_symtable_item_types(identifier_type, returned_type))
				{
					exit_with_error(SEMANTIC_ERR_EXPR, "Cannot assign %s to %s", returned_type.data_type,
									identifier_type.data_type);
				}
			}
			else
			{
		...
		SymtableItem *item = symtable_add_symbol(table, variable_id, SYMTABLE_VARIABLE, var_initialized,
												 is_constant, false);
		if (var_initialized)
		{
	...
	else if (check_type(parser, TOKEN_IDENTIFIER))
	{
		...
		if (peek(parser)->type == TOKEN_ASSIGN)
		{
			...
			SymtableItem *item = symtable_get(table, current_token(parser)->val);
